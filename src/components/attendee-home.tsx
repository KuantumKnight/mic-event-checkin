...
export function AttendeeHome() {
  ...
  async function register(event: EventItem) {
    setBusy(true);
    setNotice(null);
    const response = await fetch(`/api/events/${event.id}/register`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ displayName: "MIC attendee", email: `attendee-${Date.now()}@mic.local` }) });
