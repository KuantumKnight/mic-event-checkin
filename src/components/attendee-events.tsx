...
export function AttendeeEvents({ initialEvents, profileName }: { initialEvents: EventSummary[]; profileName: string }) {
  ...
  useEffect(() => { void fetch("/api/my-registrations").then((response) => response.ok ? response.json() : { registrations: [] }).then((body) => setRegistrations(body.registrations ?? [])).finally(() => setLoading(false)); }, []);
