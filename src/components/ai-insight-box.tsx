...
export function AiInsightBox({ eventId }: { eventId: string }) {
  ...
  return <section className="insight-box"><div className="insight-heading"><BrainCircuit size={19} /><div><span className="eyebrow">Event insight</span><h2>Ask the record.</h2></div></div><form onSubmit={ask} className="insight-form"><input value={question} onChange={(event) => setQuestion(event.target.value)} minLength={3} maxLength={500} /><button className="button button-dark" disabled={busy}>{busy ? <Loader2 size={16} className="spin" /> : <Send size={16} />} Ask</button></form>{answer && <div className="insight-answer"><p>{answer}</p>{source && <small>Source: {source === "ai" ? "AI grounded in this event's database stats" : "deterministic database fallback"}</small>}</div>}</section>;
}
