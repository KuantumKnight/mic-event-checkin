...
export async function GET(_request: Request, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  const [{ data: stats, error: statsError }, { data: attendees, error: attendeesError }] = await Promise.all([
    supabase.from("event_stats").select("id, organizer_id, name, description, starts_at, ends_at, location, capacity, registered_count, checked_in_count, spots_left, status").eq("id", eventId).eq("organizer_id", profile.id).single(),
