...
export default async function CheckInsPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const { eventId } = await params; const supabase = await createClient(); const [{ data: event }, { data: registrations }] = await Promise.all([supabase.from("events").select("id, name").eq("id", eventId).single(), supabase.from("registrations").select("id, display_name, email, checkins(checked_in_at, station_id)").eq("event_id", eventId).order("created_at", { ascending: false })]);
  if (!event) notFound();
