...
export default async function EventOverviewPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const [{ data: stats }, { data: attendees }] = await Promise.all([supabase.from("event_stats").select("id, organizer_id, name, description, starts_at, ends_at, location, capacity, registered_count, checked_in_count, spots_left, status").eq("id", eventId).eq("organizer_id", profile.id).single(), supabase.from("registrations").select("id, display_name, email, created_at, checkins(checked_in_at, station_id)").eq("event_id", eventId).order("created_at", { ascending: false }).limit(8)]);
  if (!stats) notFound();
  ...
  return <OrganizerShell profile={profile as Profile}><PageHeader eyebrow="Event overview" title="Monitor this event" description="Registration and check-in counts refresh automatically." /><EventOverview eventId={eventId} initialStats={initialStats} /></OrganizerShell>;
}
