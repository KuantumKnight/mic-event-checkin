...
export default async function EventsPage() {
  ...
  if (error) return <OrganizerShell profile={profile as Profile}><PageHeader eyebrow="Events" title="Manage events" description="The event record could not be loaded." /><DataError message="Events could not be loaded." onRetry="/events" /></OrganizerShell>;
  const events = (data ?? []) as EventSummary[];
  return <OrganizerShell profile={profile as Profile}><PageHeader eyebrow="Events" title="Manage events" description="Create events, review capacity, export registrations, and open the scanner." action={<Link className="button button-primary" href="/events/new"><Plus size={16} /> New event</Link>} /><EventList events={events} /></OrganizerShell>;
}
